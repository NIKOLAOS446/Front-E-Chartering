...
class UserProfile extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(`https://localhost:44321/api/ApplicationUser/GetUser`, {
            ...
            ;
        axios.get(`https://localhost:44321/api/Ratings/GetRatingss`, {
            headers: { Authorization: "Bearer " + this.state.token }
        }).then(res => {
            // console.log(this.props.token);
            this.setState({ rating: res.data, loading: false })
            console.log("data prod", res.data);
        })
            .catch(err => this.setState({ loading: false }))
            ;
        axios.get(`https://localhost:44321/api/Ratings/GetRatingsss`, {
            headers: { Authorization: "Bearer " + this.state.token }
        }).then(res => {
            // console.log(this.props.token);
    ...
    render() {
        return (
            <div>
                <UserConsumer>{(token) => { this.state.token = token }
                }</UserConsumer>
                    ...
                    <div className="right">
                        <div className="info">
                            ...
                            <div className="info_data">
                                ...
                                {(this.state.rating != 0) ? <div className="data">
                                    <h4>Rating</h4>
                                    <StarRating rating={this.state.rating} user={this.state.user}/>
                                </div>
                                    : null}
                              {/* { (this.state.rating != 0) ? (this.state.Countratings.map(Countrating=>
                                <div className="data">
                                <h4> {Countrating.description}</h4>
                                <p>{Countrating.count}</p>
                            </div>
                                )): null}  
                                 */}
                            </div>
